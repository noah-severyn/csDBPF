...
namespace csDBPF.EntryTypes {
	internal class EntryEXMP : DBPFEntry {
		...
		public EntryEXMP(DBPFTGI tgi, uint offset, uint size, uint index) : base(tgi, offset, size, index) {
		...
		public int GetExemplarType() {
			if (!(base.TGI.MatchesKnownTGI(DBPFTGI.EXEMPLAR) || base.TGI.MatchesKnownTGI(DBPFTGI.COHORT))) {
				return -1;
			}
		...
		public override List<DBPFProperty> DecodeEntry() {

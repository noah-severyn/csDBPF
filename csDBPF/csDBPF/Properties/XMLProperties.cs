...
namespace csDBPF.Properties {
	...
	public static class XMLProperties {
        ...
        static XMLProperties() {
            ...
            //string xmlPath = Path.Combine(outPutDirectory, "Properties\\new_properties.xml");
			...
			foreach (XElement prop in exemplarProperties) {
                ...
                //For the rest of the attributes, we do not know which ones will exist for a property ahead of time, so directly setting them from the constructor results in a null reference exception. Examine then set each one individually.
                //See https://stackoverflow.com/a/44929328/10802255. This is kind of hideous, but we are first checking if the XAttribute exists. If it does exist then a string value is returned; null is returned if it does not exist. Throw this returned value into the TryParse. Since we have to deal with the possibility of the value being null and TryParse can only out non-nullable values, we need to check the result of TryParse whether it was successful or not. If it was successful we just return the out value, if it was not successful we finally just return null. 
		...
		public static XMLExemplarProperty GetXMLProperty(uint id) {
			IEnumerable<XMLExemplarProperty> matchingPropery = from prop in _allProperties
                                                               where prop.ID == id
															   select prop;
			return matchingPropery.First(); //Because of the filter there will always only be one result
        }
		/// <summary>
		/// Queries new_properties.xml and returns the exemplar property (PROPERTY) element matching the specified Name.
		/// </summary>
		/// <param name="name">Name to lookup. Case INsensitive. Spaces are ignored</param>
		/// <returns>A matching XMLExemplarProperty</returns>
		/// <remarks>
		/// Spaces and case sensitivity are ignored due to inconsistencies in how the Name field is saved in the xml file.
		/// </remarks>
		...
		public static uint GetPropertyID(string nameToFind) {
			foreach (XMLExemplarProperty property in _allProperties) {

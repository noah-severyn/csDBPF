    ...
    public class DBPFUnitTests {
        ...
        public class _03x_ByteArrayHelper {
            ...
            public void Test_031b_ByteArrayHelper_ToBytes_String() {
                ...
                CollectionAssert.AreEqual(b1, s1.ToBytes(true));
                CollectionAssert.AreEqual(b2, ByteArrayHelper.ToBytes(s2, true));
                CollectionAssert.AreEqual(b2, s2.ToBytes(true));
                ...
                CollectionAssert.AreEqual(b3, s3.ToBytes());
                CollectionAssert.AreEqual(b3_single, ByteArrayHelper.ToBytes(s3, true));
                CollectionAssert.AreEqual(b3_single, s3.ToBytes(true));
                ...
                CollectionAssert.AreEqual(b4, ByteArrayHelper.ToBytes(s4));
                CollectionAssert.AreEqual(b4, s4.ToBytes());
                Assert.AreEqual(s4, System.Text.Encoding.Unicode.GetString(b4));
            }

            [TestMethod]
            public void Test_033a_ByteArrayHelper_ReadIntoUshort() {
                byte[] dbpf = [0x44, 0x42, 0x50, 0x46];
                Assert.AreEqual((ushort) 0x4442, ByteArrayHelper.ReadIntoUshort(dbpf, 0, DBPF.Encoding.Binary));
                Assert.AreEqual((ushort) 0x4442, dbpf.ReadIntoUshort(0, DBPF.Encoding.Binary));
                Assert.AreEqual((ushort) 0x5046, ByteArrayHelper.ReadIntoUshort(dbpf, 2, DBPF.Encoding.Binary));
                Assert.AreEqual((ushort) 0x5046, dbpf.ReadIntoUshort(2, DBPF.Encoding.Binary));

                byte[] arr = [0x31, 0x32, 0x33, 0x34];
                Assert.AreEqual((ushort) 0x1234, ByteArrayHelper.ReadIntoUshort(arr, 0, DBPF.Encoding.Text));
                Assert.AreEqual((ushort) 0x1234, arr.ReadIntoUshort(0, DBPF.Encoding.Text));
                Assert.AreEqual((ushort) 0x3132, ByteArrayHelper.ReadIntoUshort(arr, 0, DBPF.Encoding.Binary));
                Assert.AreEqual((ushort) 0x3132, arr.ReadIntoUshort(0, DBPF.Encoding.Binary));

                byte[] arrZeros = [0x30, 0x30, 0x30, 0x30];
                Assert.AreEqual((ushort) 0x00, ByteArrayHelper.ReadIntoUshort(arrZeros, 0, DBPF.Encoding.Text));
                Assert.AreEqual((ushort) 0x3030, ByteArrayHelper.ReadIntoUshort(arrZeros, 0, DBPF.Encoding.Binary));
            }

            [TestMethod]
            public void Test_033b_ByteArrayHelper_ReadIntoInt() {
                byte[] dbpf = [0x44, 0x42, 0x50, 0x46];
                Assert.AreEqual(0x44425046, ByteArrayHelper.ReadIntoInt(dbpf, 0, DBPF.Encoding.Binary));
                Assert.AreEqual(0x44425046, dbpf.ReadIntoInt(0, DBPF.Encoding.Binary));

                byte[] arr = [0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38];
                Assert.AreEqual(0x12345678, ByteArrayHelper.ReadIntoInt(arr, 0, DBPF.Encoding.Text));
                Assert.AreEqual(0x31323334, ByteArrayHelper.ReadIntoInt(arr, 0, DBPF.Encoding.Binary));
                Assert.AreEqual(0x35363738, ByteArrayHelper.ReadIntoInt(arr, 4, DBPF.Encoding.Binary));

                byte[] arr2 = [0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x33];
                Assert.AreEqual(0x03, ByteArrayHelper.ReadIntoInt(arr2, 0, DBPF.Encoding.Text));
                Assert.AreEqual(0x30303030, ByteArrayHelper.ReadIntoInt(arr2, 0, DBPF.Encoding.Binary));
                Assert.AreEqual(0x30303033, ByteArrayHelper.ReadIntoInt(arr2, 4, DBPF.Encoding.Binary));
            }


            [TestMethod]
            public void Test_033c_ByteArrayHelper_ReadIntoUint() {
                byte[] dbpf = [0x44, 0x42, 0x50, 0x46];
                Assert.AreEqual((uint) 0x44425046, ByteArrayHelper.ReadIntoUint(dbpf, 0, DBPF.Encoding.Binary));
                Assert.AreEqual((uint) 0x44425046, dbpf.ReadIntoUint(0, DBPF.Encoding.Binary));
                
                byte[] arr1 = [0x00, 0x00, 0x10, 0x00];
                Assert.AreEqual((uint) 0x00001000, ByteArrayHelper.ReadIntoUint(arr1, 0, DBPF.Encoding.Binary));
                Assert.AreEqual((uint) 0x00001000, arr1.ReadIntoUint(0, DBPF.Encoding.Binary));
                
                byte[] arr2 = [0x07, 0x00, 0x00, 0x30];
                Assert.AreEqual((uint) 0x07000030, ByteArrayHelper.ReadIntoUint(arr2, 0, DBPF.Encoding.Binary));
                Assert.AreEqual((uint) 0x07000030, arr2.ReadIntoUint(0, DBPF.Encoding.Binary));

                byte[] arr3 = [0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30];
                Assert.AreEqual((uint) 0x00, ByteArrayHelper.ReadIntoUint(arr3, 0, DBPF.Encoding.Text));
                Assert.AreEqual((uint) 0x00, arr3.ReadIntoUint(0, DBPF.Encoding.Text));
                Assert.AreEqual((uint) 0x30303030, ByteArrayHelper.ReadIntoUint(arr3, 0, DBPF.Encoding.Binary));
                Assert.AreEqual((uint) 0x30303030, arr3.ReadIntoUint(0, DBPF.Encoding.Binary));

                byte[] arr4 = [0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38];
                Assert.AreEqual((uint) 0x12345678, ByteArrayHelper.ReadIntoUint(arr4, 0, DBPF.Encoding.Text));
                Assert.AreEqual((uint) 0x12345678, arr4.ReadIntoUint(0, DBPF.Encoding.Text));
                Assert.AreEqual((uint) 0x31323334, ByteArrayHelper.ReadIntoUint(arr4, 0, DBPF.Encoding.Binary));
                Assert.AreEqual((uint) 0x31323334, arr4.ReadIntoUint(0, DBPF.Encoding.Binary));
            }

            [TestMethod]
            public void Test_033d_ByteArrayHelper_ReadIntoLong() {
                byte[] dbpf = [0x70, 0x99, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
                Assert.AreEqual(0x7099000000000000, ByteArrayHelper.ReadIntoLong(dbpf, 0, DBPF.Encoding.Binary));
                Assert.AreEqual(0x7099000000000000, dbpf.ReadIntoLong(0, DBPF.Encoding.Binary));

                byte[] arr = [0x30, 0x30, 0x65, 0x62, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x35, 0x34, 0x33, 0x32, 0x31];
                Assert.AreEqual(0x00eb000000054321, arr.ReadIntoLong(0, DBPF.Encoding.Text));
            }

            [TestMethod]
            public void Test_033e_ByteArrayHelper_ReadIntoFloat() {
                // Binary: 1.0f -> 0x3F800000 (big-endian)
                byte[] oneBytes = [0x3F, 0x80, 0x00, 0x00];
                Assert.AreEqual(1.0f, ByteArrayHelper.ReadIntoFloat(oneBytes, 0, 4, DBPF.Encoding.Binary));
                Assert.AreEqual(1.0f, oneBytes.ReadIntoFloat(0, 4, DBPF.Encoding.Binary));

                // Text: "0.5"
                byte[] halfText = System.Text.Encoding.ASCII.GetBytes("0.5");
                Assert.AreEqual(0.5f, ByteArrayHelper.ReadIntoFloat(halfText, 0, halfText.Length, DBPF.Encoding.Text));

                // Non-numeric text -> float.TryParse will fail and method returns 0
                byte[] badText = System.Text.Encoding.ASCII.GetBytes("abcd");
                Assert.AreEqual(0f, ByteArrayHelper.ReadIntoFloat(badText, 0, badText.Length, DBPF.Encoding.Text));
            }


            [TestMethod]
            public void Test_034_ByteArrayHelper_ToAString() {
                byte[] dbpfB = [0x44, 0x42, 0x50, 0x46];
                byte[] dbpfB1 = [68, 66, 80, 70];
                Assert.AreEqual("DBPF", ByteArrayHelper.ToAString(dbpfB));
                Assert.AreEqual("DBPF", dbpfB.ToAString());
                Assert.AreEqual(ByteArrayHelper.ToAString(dbpfB), ByteArrayHelper.ToAString(dbpfB1));
                Assert.AreEqual(dbpfB.ToAString(), dbpfB1.ToAString());
                Assert.AreEqual("DBPF", ByteArrayHelper.ToAString(dbpfB, 0));
                Assert.AreEqual("DBPF", dbpfB.ToAString(0));
                Assert.AreEqual("DBPF", ByteArrayHelper.ToAString(dbpfB, 0, dbpfB.Length));
                Assert.AreEqual("DBPF", dbpfB.ToAString(0, dbpfB.Length));
                Assert.AreEqual("BPF", ByteArrayHelper.ToAString(dbpfB, 1));
                Assert.AreEqual("BPF", dbpfB.ToAString(1));
                Assert.AreEqual("DB", ByteArrayHelper.ToAString(dbpfB, 0, 2));
                Assert.AreEqual("DB", dbpfB.ToAString(0, 2));
                Assert.AreEqual("P", ByteArrayHelper.ToAString(dbpfB, 2, 1));
                Assert.AreEqual("P", dbpfB.ToAString(2, 1));


                string lettersS = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
                byte[] lettersB = [0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6A, 0x6B, 0x6C, 0x6D, 0x6E, 0x6F, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7A, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x4E, 0x4F, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5A];
                Assert.AreEqual(lettersS, ByteArrayHelper.ToAString(lettersB));
                Assert.AreEqual(lettersS, lettersB.ToAString());

                string numbersS = "01213456789";
                byte[] numbersB = [0x30, 0x31, 0x32, 0x31, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39];
                Assert.AreEqual(numbersS, ByteArrayHelper.ToAString(numbersB));
                Assert.AreEqual(numbersS, numbersB.ToAString());

                string specialS = "~`!@#$%^&*()_+-=[]\\{}|;':\",./<>?";
                byte[] specialB = [0x7E, 0x60, 0x21, 0x40, 0x23, 0x24, 0x25, 0x5E, 0x26, 0x2A, 0x28, 0x29, 0x5F, 0x2B, 0x2D, 0x3D, 0x5B, 0x5D, 0x5C, 0x7B, 0x7D, 0x7C, 0x3B, 0x27, 0x3A, 0x22, 0x2C, 0x2E, 0x2F, 0x3C, 0x3E, 0x3F];
                Assert.AreEqual(specialS, ByteArrayHelper.ToAString(specialB));
                Assert.AreEqual(specialS, specialB.ToAString());

                Assert.AreEqual(string.Empty, ByteArrayHelper.ToAString(null));
            }
        }
        ...
        public class _06x_DBPFProperty {
            ...
            public void Test_061a_DBPFPropertyString_Binary() {
                ...
                CollectionAssert.AreEqual(stringdataviewparksaura.ToArray(), propb.GetTypedData());
                ...
                CollectionAssert.AreEqual(knownprop.GetTypedData(), propb.GetTypedData());
                ...
                propb.SetTypedData(stringparksaura.ToArray());
                CollectionAssert.AreEqual(stringparksaura.ToArray(), propb.GetTypedData());
                Assert.AreEqual(stringparksaura.Length, propb.NumberOfReps);
                propb.SetTypedData(stringparks.ToArray());
                CollectionAssert.AreEqual(stringparks.ToArray(), propb.GetTypedData());
            ...
            public void Test_061b_DBPFPropertyLong_Binary() {
                ...
                //28 UInt32s
                vals = new uint[28] { 0x00000000, 0x70990000, 0x00000001, 0x70BC533C, 0x0000000C, 0x70BC533C, 0x0000000D, 0x70D98C79, 0x00000046, 0x70D98C79, 0x0000007F, 0x70F0C5BA, 0x00000080, 0x70FFFFFF, 0x00000081, 0x70E2F1DD, 0x000000B8, 0x70C5E3BB, 0x000000B9, 0x70A8D49A, 0x000000F2, 0x708AC679, 0x000000F3, 0x706AB758, 0x000000FE, 0x7046A836, 0x000000FF, 0x70009900 };
            ...
            public void Test_061d_DBPFPropertyString_Text() {
                ...
                Assert.AreEqual(val, new string((ReadOnlySpan<char>) propt.GetTypedData()));
                ...
                CollectionAssert.AreEqual(propknown.GetTypedData(), propt.GetTypedData());
            }
            ...
            public void Test_061e_DBPFPropertyLong_Text() {
                ...
                CollectionAssert.AreEqual(vals, propt.GetTypedData());
                Assert.AreEqual(propknown.NumberOfReps, propt.NumberOfReps);
                Assert.AreEqual(propknown.DataType, propt.DataType);
                CollectionAssert.AreEqual(propknown.GetTypedData(), propt.GetTypedData());

                //1x Sint64
                vals = new long[1] { 0x00000000000000A9 };
                ...
                //4x Sint32
                vals = new int[4] { 0x07, 0x03, 0x16, 0x00 };
                ...
                //4x Uint32
                vals = new uint[4] { 0x1001, 0x2000, 0x2001, 0x13110 };
            ...
            public void Test_061f_DBPFPropertyFloat_Text() {
                ...
                //3x Float32
                vals = [81.589798f, 13.947300f, 39.442501f];
            ...
            public void Test_062_DBPFProperty_DecodeNoProperties() {
                ...
                entry.Decode();
                Assert.IsEmpty(entry.ListOfProperties);

                entry = new DBPFEntryEXMP(DBPFTGI.EXEMPLAR, 0, 0, 0, TestArrays.entrynullproperty_extradata);
                Assert.IsEmpty(entry.ListOfProperties);

                //Use an IRL example
                DBPFFile jimspack = new DBPFFile("C:\\Users\\Administrator\\OneDrive\\SC4 Deps\\new\\Jim CarProp Pack 1.2\\Jim CarProp Pack 1.2.dat");
        ...
        public class _07x_XMLProperty {
            ...
            public void Test_070_XMLProperties_AllProperties() {
                Assert.HasCount(0x56f, XMLProperties.AllProperties);
            }
        ...
        public class _08x_DBPFEntry {
            ...
            public class _082_DIR {
                ...
                public void Test_083a_DIR_Decode() {
                    ...
                    entryd.Decode();
                    Assert.HasCount(1, entryd.CompressedItems);
                    ...
                    dir.Decode();
                    Assert.HasCount(1, dir.CompressedItems);
                ...
                public void Test_083b_DIR_Build() {
                    ...
                    Assert.HasCount(7, dir2.CompressedItems);
                    ...
                    foreach (DBPFEntryFSH entry in entries.Cast<DBPFEntryFSH>()) {
			            ...
			            foreach (var fshEntry in entry.Entries) {
				            foreach (var mipmap in fshEntry.Mipmaps) {
					            ...
					            mipmap.Decompress();
                                Assert.HasCount(width * height * 4, mipmap.Bitmap);
                            }
			            } 
		            }
	            }
        ...
        public class _1xx_DBPFFile {
            ...
            public void Test_101b_DBPFFile_IsNotDBPF() {
                ...
                Assert.IsEmpty(notdbpf.ListOfEntries);
            }
            ...
            public void Test_111_ParseAllEntries() {
                ...
                for (int idx = 0; idx < entryKnown.ListOfProperties.Count; idx++) {
                    ...
                    CollectionAssert.AreEqual(outk.GetTypedData(), outr.GetTypedData());
                }
